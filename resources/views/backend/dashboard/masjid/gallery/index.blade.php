...
@section('content')
            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap py-5">
                    <div class="card-title">
                        <h3 class="card-label">Masjid Galleries List
                            <span class="d-block text-muted pt-2 font-size-sm">All gallery here</span>
                        </h3>
                    </div>
                        ...
                        <tbody>
                            @foreach ($galleries as $gallery)
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{ asset($gallery->image) }}" style="width: 150px;" alt="">
                                    </td>
                                    <td>{{ $gallery->order }}</td>
                                    <td>
                                        @if ($gallery->status == 1)
                                            <a href="#" class="btn label label-lg label-light-success label-inline"
                                                data-toggle="modal"
                                                @if (Auth('admin')->user()) data-target="#row_status_{{ $gallery->id }}" @endif>
                                                Active</a>
                                    ...
                                    @if (Auth('admin')->user())
                                        <td class="d-flex">
                                            <a href="#" data-toggle="modal"
                                                data-target="#edit_campus_{{ $gallery->id }}"
                                                class="btn btn-icon btn-info btn-hover-primary btn-xs mx-3"><i
                                                    class="fa fa-edit"></i></a>
                                            <a id="delete" href="{{ route('admin.masjid.gallery.destroy', $gallery->id) }}"
                                ...
                                <!--Row Status -->
                                <div class="modal fade" data-backdrop="static" id="row_status_{{ $gallery->id }}"
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ route('admin.masjid.gallery.status', $gallery->id) }}" method="post">
                                                @csrf
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="status" class="form-control">
                                                            <option value="1"
                                                                @if ($gallery->status == 1) selected @endif>Active
                                                            </option>
                                                            <option value="0"
                                                                @if ($gallery->status == 0) selected @endif>
                                                                Inactive</option>
                                                        </select>
                                ...
                                <!-- Edit Modal -->
                                <div class="modal fade" id="edit_campus_{{ $gallery->id }}" tabindex="-1" role="dialog"
    ...
    <div class="modal fade" id="addmodal" tabindex="-1" role="dialog" data-backdrop="static"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Create Gallery Image</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="fa fa-close"></i>
                    </button>
                </div>
                <form action="{{ route('admin.masjid.gallery.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
